#' @templateVar MODEL_FUNCTION igt_pvl_delta_regression
...
#' @templateVar MODEL_CODE pvl_delta_regression
#' @templateVar MODEL_CITE (Ahn et al., 2014)
#' @templateVar MODEL_TYPE Hierarchical
#' @templateVar DATA_COLUMNS "subjID", "choice", "gain", "loss"
#' @templateVar PARAMETERS \code{A} (delta rate), \code{alpha} (outcome sensitivity), \code{cons} (response consistency), \code{lambda} (loss aversion)
...
#' data("igt_example")
#'
#' output <- igt_pvl_delta_regression(
#'   data = igt_example,
#'   exclude_cols = "trial",
...
igt_pvl_delta_regression <- hBayesDMregression_model(
  task_name       = "igt",
  model_name      = "pvl_delta_regression",
  model_type      = "",
