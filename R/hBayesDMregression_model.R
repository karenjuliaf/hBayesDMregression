#' hBayesDMregression Model Function
#'
#' @description
#' The base function from which all hBayesDMregression model functions are created. Derived from the
...
#'
#' @return A specific hBayesDMregression model function.

hBayesDMregression_model <- function(task_name,
                                      model_name,
                                      model_type = "",
                                      data_columns,
                                      parameters,
  ...
  function(data            = NULL,
    ...
    # Check if regression parameters have been specified
    if (is.null(regression_pars)) {
      stop("No regression parameters supplied. Use task functions found in package `hBayesDMregression`
           instead.")
    }

    # Check that supplied names correspond to model parameters
    if (!all(regression_pars %in% names(parameters))) {
    ...
    # Reorder `regression_pars` to match the order of appearance in Stan code
    regression_pars <- reorder_regression_pars(model_name, regression_pars)

    # Preprocess the raw data to pass to Stan
    data_list <- preprocess_func(raw_data, general_info, regression_pars, ...)
    ...
    if (is.null(stanmodel_arg)) {
      stanmodel_arg <- rstan::stan_model(model_code = write_stan_model(model_name, regression_pars))
    ...
    if (inits[1] == "vb") {
      if (vb) {
        ...
        make_gen_init_from_vb <- function() {
          fit_vb <- rstan::vb(object = stanmodel_arg, data = data_list, init_r = 1e-6)
          m_vb <- colMeans(as.data.frame(fit_vb))

          function() {
            ret <- list(
              mu_pr = as.vector(m_vb[startsWith(names(m_vb), "mu_pr")]),
              sigma = as.vector(m_vb[grepl("sigma\\[", names(m_vb))])
            )
            ...
            ret[["sigma_beta"]] <- matrix(
              m_vb[startsWith(names(m_vb), "sigma_beta")],
              nrow = length(regression_pars), ncol = data_list$ncov, byrow = TRUE
            )
    ...
    # Convert parVals$beta and parVals$sigma_beta into matrices with clear column names
    parVals$beta <- array_to_mat(
      parVals$beta, model_name, regression_pars,
      colnames(data_list$covariate_matrix), sigma=FALSE
    )
    parVals$sigma_beta <- array_to_mat(
      parVals$sigma_beta, model_name, regression_pars,
      colnames(data_list$covariate_matrix), sigma=TRUE
    )
    ...
    for (i in 1:length(fit@sim$samples)) {
      ...
      # Rename the `beta`s inside samples with clear names
      beta_names_key <- names_key(regression_pars, colnames(data_list$covariate_matrix), sigma=FALSE)
      names(fit@sim$samples[[i]]) <- rename_by_key(fit@sim$samples[[i]], beta_names_key)

      # Rename the `sigma_beta`s inside samples wtih clear names
      sigma_beta_names_key <- names_key(regression_pars, colnames(data_list$covariate_matrix), sigma=TRUE)
      names(fit@sim$samples[[i]]) <- rename_by_key(fit@sim$samples[[i]], sigma_beta_names_key)
    }

    # Rename the `beta`s and `sigma_beta`s inside the stanfit object for other rstan methods
    names(fit) <- rename_by_key(fit, beta_names_key)
    names(fit) <- rename_by_key(fit, sigma_beta_names_key)
    ...
    modelData$modelPars         <- names(parameters)
    modelData$allIndPars        <- allIndPars
    modelData$parVals           <- parVals
    modelData$fit               <- fit
    modelData$subjID            <- sort(as.character(unique(raw_data$subjID)))
    modelData$rawdata           <- raw_data
    if (modelRegressor) {
      modelData$modelRegressor  <- model_regressor
    }
    modelData$model_code        <- stanmodel_arg@model_code

    # Object class definition
    class(modelData) <- "hBayesDMregression"
